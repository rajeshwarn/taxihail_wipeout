    ...
    public class DeploymentJobService
    {
        ...
        private void DeployDataBase(string packagesDirectory, string companyName, string appPoolName)
        {
	        ...
	        var p = new DatabaseInitializerParams
            {
                ...
                CompanyConnectionString = string.Format(ConfigurationManager.AppSettings["CompanyConnectionString"], companyName),
                MasterConnectionString = ConfigurationManager.ConnectionStrings["MasterConnectionString"].ConnectionString,
