...
namespace DeploymentServiceTools
{
    ...
    {
        public string CreateNewVersion(string companyKey, string versionNumber, string websiteUrl, string ipaFileName,
            FileStream ipaFile, string apkFileName, FileStream apkFile)
        {
            ...
            using (var client = new HttpClient())
            {
                using (var multipartFormDataContent = new MultipartFormDataContent())
                {
                    ...
                    {
                        message = string.Format("Version could not be created: HttpError: {0}",
