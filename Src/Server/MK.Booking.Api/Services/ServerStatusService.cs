...
namespace apcurium.MK.Booking.Api.Services
{
    public class ServerStatusService : Service
    {
        ...
        private void RunPapiTest(CmtPaymentSettings settings)
        {
            ...
            var result = CmtPaymentClient.TestClient(settings, cc.Number, cc.ExpirationDate, _logger, default(ServiceType));
            if (result)
            {
                return;
            }
        ...
        {
            var cmtMobileServiceClient = new CmtMobileServiceClient(_serverSettings.GetPaymentSettings().CmtPaymentSettings, default(ServiceType), null, null, null);
