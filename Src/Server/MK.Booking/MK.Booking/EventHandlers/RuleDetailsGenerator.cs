...
{
    public class RuleDetailsGenerator : IEventHandler<RuleCreated>, IEventHandler<RuleUpdated>, IEventHandler<RuleDeleted>, IEventHandler<RuleActivated>, IEventHandler<RuleDeactivated>
    {
        ...
        public void Handle(RuleCreated @event)
        {
            using (var context = _contextFactory.Invoke())
            {
                context.Save(new RuleDetail
                {
                    ...
                    ActiveFrom = @event.ActiveFrom.HasValue ? @event.ActiveFrom < (DateTime)SqlDateTime.MinValue ? (DateTime)SqlDateTime.MinValue : @event.ActiveFrom : null,
                    ActiveTo = @event.ActiveTo.HasValue ? @event.ActiveTo < (DateTime)SqlDateTime.MinValue ? (DateTime)SqlDateTime.MinValue : @event.ActiveTo : null,
        ...
        public void Handle(RuleUpdated @event)
            {
                using (var context = _contextFactory.Invoke())
                {
                    ...
                    rule.ActiveFrom = @event.ActiveFrom.HasValue ? @event.ActiveFrom < (DateTime)SqlDateTime.MinValue ? (DateTime)SqlDateTime.MinValue : @event.ActiveFrom : null;
                    rule.ActiveTo = @event.ActiveTo.HasValue ? @event.ActiveTo < (DateTime)SqlDateTime.MinValue ? (DateTime)SqlDateTime.MinValue : @event.ActiveTo : null;
