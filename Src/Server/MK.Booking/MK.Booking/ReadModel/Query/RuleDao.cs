...
namespace apcurium.MK.Booking.ReadModel.Query
{
    public class RuleDao : IRuleDao
    {
        ...
        public RuleDetail GetActiveDisableRule(bool isFutureBooking, DateTime pickupDate, string zone)
        {
            using (var context = _contextFactory.Invoke())
            {
        ...
        public RuleDetail GetActiveWarningRule(bool isFutureBooking, DateTime pickupDate, string zone)
        {
            using (var context = _contextFactory.Invoke())
            {
                ...
                return (from r in rule where !string.IsNullOrEmpty(r.ZoneList) let zoneList = r.ZoneList.Split(',') where zoneList.SingleOrDefault(c=>c.Equals(zone)).HasValue() select r).FirstOrDefault();
            }
        }
