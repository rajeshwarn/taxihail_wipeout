...
namespace apcurium.MK.Booking.Test.FavoriteAddressesFixture
{
    ...
    public class given_an_address
    {
        ...
        public void Setup()
        {
            ...
            this.sut.Given(new AddressAdded { AddressId = _addressId, SourceId = _accountId, FriendlyName = "Chez François", Apartment = "3939", FullAddress = "1234 rue Saint-Hubert", RingCode = "3131", Latitude = 45.515065, Longitude = -73.558064 });
        }

        [Test]
        public void when_address_removed_successfully()
        {
            this.sut.When(new RemoveAddress { AccountId = _accountId, AddressId = _addressId });

            Assert.AreEqual(1, sut.Events.Count);
            var evt = (AddressRemoved) sut.Events.Single();
        ...
        {
            Assert.Throws<InvalidOperationException>(() => this.sut.When(new RemoveAddress
            {
        ...
        {
            this.sut.When(new UpdateAddress { AccountId = _accountId, AddressId = _addressId, FriendlyName = "Chez Costo", FullAddress = "1234 rue Saint-Hubert" });

            Assert.AreEqual(1, sut.Events.Count());
            var evt = (AddressUpdated)sut.Events.Single();
            Assert.AreEqual(_accountId, evt.SourceId);
            Assert.AreEqual(_addressId, evt.AddressId);
        }
        ...
        {
            Assert.Throws<InvalidOperationException>(() => this.sut.When(new UpdateAddress { AccountId = _accountId, AddressId = _addressId, FriendlyName = "Chez Costo"}));
        }
    }
}
