...
namespace CustomerPortal.Web.Test.Helpers
{
    public static class MongoMock
    {
        public static Mock<MongoServer> CreateMongoServer()
        {
            ...
            var server = new Mock<MongoServer>(MockBehavior.Default, serverSettings);
        ...
        public static Mock<MongoDatabase> CreateMongoDatabase(MongoServer server)
        {
            ...
            var database = new Mock<MongoDatabase>(MockBehavior.Default, server, "MockDatabase", databaseSettings);
        ...
        public static Mock<MongoCollection<T>> CreateMongoCollection<T>(MongoDatabase database, string collectionName)
        {
            ...
            var collectionMock = new Mock<MongoCollection<T>>(MockBehavior.Default, database, collectionName, collectionSetting);
        ...
        {
            var cursorMock = new Mock<MongoCursor<T>>(MockBehavior.Default, collection, null, null, null, null);
