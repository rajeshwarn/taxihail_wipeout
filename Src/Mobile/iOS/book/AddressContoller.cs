...
using TaxiMobileApp.Lib.Data;
using TaxiMobileApp.Controls;
...
namespace TaxiMobileApp
{
	public class AddressContoller
	{
		...
		public AddressContoller (UITextField text, UITextField apt, UITextField ringCode, UITableView table, VerticalButtonBar bar, MKMapView map, AddressAnnotationType adrsType, 
...
//			_btn.Hidden = _text.Text.HasValue ();
		...
		void PickAddressTouchUpInside (int index)
		{
			...
			case 2:
				...
				break;
			case 3:
				var googlePlaceService = new GooglePlacesService( ServiceLocator.Current.GetInstance<IAppResource> ().CurrentLanguageCode );
				var places = googlePlaceService.GetNearbyPlaces( _map.CenterCoordinate.Latitude, _map.CenterCoordinate.Longitude );
				var locations = places.Select( p => DataMapper.GooglePlaceDataToLocationData( p ) ).ToList();
		...
		private void DoLocationChange( LocationData locData )
		{
			try
			{
