				...
				try
				{
					...
					if (status != null)
					{
						...
						if ((position.Latitude != 0) && (position.Longitude != 0))
						{
							...
							{
								InvokeOnMainThread (() => { mapStatus.RemoveAnnotations ( mapStatus.Annotations.OfType<AddressAnnotation>().Where ( a=>a.AddressType == AddressAnnotationType.Taxi ).ToArray ()); });
							}
