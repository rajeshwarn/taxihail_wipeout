...
namespace apcurium.MK.Booking.Mobile.Client.Controls.Message
{
    public class LoadingOverlayView: UIView
    {
        ...
        private bool IsTaxiAssetOverriden(string imagePath, UIColor expectedColor, Point expectedColorCoordinate)
        {
            var asset = ImageHelper.GetImage(imagePath);
            var detectedColor = GetPixel(asset, expectedColorCoordinate.X, expectedColorCoordinate.Y);
            return !detectedColor.CGColor.Equals(expectedColor.CGColor);
        }
		
		private UIColor GetPixel(UIImage image, int x, int y)
		{
