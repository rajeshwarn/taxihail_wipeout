...
//			Right = right;
	...
	public class EntryElement : Element {
		...
		public UITextAutocapitalizationType? TextAutocapitalizationType = null;
		public UITextAutocorrectionType TextAutocorrectionType = UITextAutocorrectionType.Default;		
		...
		public override UITableViewCell GetCell (UITableView tv)
		{
			...
			if (entry == null){
				...
				var _entry = new UITextField (new RectangleF (size.Width, (cell.ContentView.Bounds.Height-size.Height)/2-1, 320-size.Width - 30, size.Height)){
					Tag = 1,
					Placeholder = placeholder ?? "",
					SecureTextEntry = isPassword
				};
				_entry.TextAlignment = UITextAlignment.Right;
				_entry.Text = Value ?? "";
				entry = _entry;
				...
				entry.ShouldReturn += //delegate {
					delegate(UITextField textField) { return textField.ResignFirstResponder (); };
...
//				};
				entry.Started += delegate {
					...
					if ( TextAutocapitalizationType.HasValue )
					{
						entry.AutocapitalizationType = TextAutocapitalizationType.Value; 	
					}
					
					entry.AutocorrectionType = TextAutocorrectionType;
					
						
				};
			}
			
			//entry.MinimumFontSize = 10;
			entry.BackgroundColor  =UIColor.Clear;
			
			if ( OffsetX.HasValue )
			{
				entry.Frame = new RectangleF( 140 - OffsetX.Value, entry.Frame.Y, 150 + OffsetX.Value, entry.Frame.Height );
			}
//			entry.Layer.BorderColor = UIColor.Red.CGColor;
