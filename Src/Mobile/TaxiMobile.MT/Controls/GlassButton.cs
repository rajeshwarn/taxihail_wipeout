...
namespace TaxiMobile.Controls
{
	...
	public class GlassButton : UIButton
	{
		...
		public static GlassButton Wrap (UIButton button, UIColor normal, UIColor pressed)
		{
			button.Hidden = true;
			
			var btnGlossy = new GlassButton (button.Frame);
			btnGlossy.Font = button.Font;
			btnGlossy.NormalColor = normal; 
			btnGlossy.HighlightedColor = pressed;
			btnGlossy.SetTitle (button.Title (UIControlState.Normal), UIControlState.Normal);
			button.Superview.AddSubview (btnGlossy);
			return btnGlossy;
		}
		...
		public override void Draw (RectangleF rect)
		{
			...
			CGPath container;
			container = GraphicsUtil.MakeRoundedRectPath2 (bounds, 8);
			...
			using (var cs = CGColorSpace.CreateDeviceRGB ())
			{
				...
				container = GraphicsUtil.MakeRoundedRectPath2( bounds.Inset( 1f, 1f), 4f);
				context.AddPath (container);
				context.Clip ();
				...
				var nb = bounds.Inset( 3f, 3f);
				container = GraphicsUtil.MakeRoundedRectPath2 (nb, 6);
				context.AddPath (container);
				context.Clip ();
