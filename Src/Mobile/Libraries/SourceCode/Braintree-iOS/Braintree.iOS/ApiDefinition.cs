...
//	}
	...
	// Needed to avoid Action in a Action issue.
	delegate void ActionWithPaymentMethodNonceAndNSError(BTPaymentMethodNonce paymentNonce, NSError NSError);
	...
	interface BTTokenizationService
	{
		...
		void RegisterType (string type, Action<BTAPIClient, NSDictionary, ActionWithPaymentMethodNonceAndNSError> tokenizationBlock);
...
//	}
	...
	interface BTHTTP : INSCopying
	{
		...
		[DesignatedInitializer]
		IntPtr ConstructorWithAuthorizationFingerprint (NSUrl URL, string authorizationFingerprint);
		...
		[DesignatedInitializer]
		IntPtr ConstructorWithTokenizationKey (NSUrl URL, string tokenizationKey);
	...
	// @interface  (BTApplePayClient)
	//// [Category]
	[BaseType (typeof(BTApplePayClient))]
	interface BTApplePayClient_
	{
		// @property (nonatomic, strong) BTAPIClient * apiClient;
		[Export ("apiClient")]
		BTAPIClient ApiClient { get; set; }
...
//	}
	...
	interface BTDropInSelectPaymentMethodViewController
	{
		...
		[Wrap ("WeakDelegate")]
		[NullAllowed]
		BTDropInSelectPaymentMethodViewController Delegate { get; set; }
	...
	interface BTPaymentButton_
	{
		...
		NSOrderedSet FilteredEnabledPaymentOptions { get; }
	}
...
//	}
	...
	interface PayPalOneTouchCore
	{
		...
		[Static]
		[Export ("clientMetadataID:")]
		string ClientMetadataID (string pairingId);
	...
	interface BTThreeDSecureLookupResult
	{
		...
		bool IsUserAuthenticationRequired { get; }
	}
	...
	// @interface  (BTThreeDSecureDriver)
	// [Category]
	[BaseType (typeof(BTThreeDSecureDriver))]
	interface BTThreeDSecureDriver_
	{
		// @property (nonatomic, strong) BTAPIClient * apiClient;
		[Export ("apiClient")]
		BTAPIClient ApiClient { get; set; }
		...
		// @property (nonatomic, strong) BTThreeDSecureCardNonce * upgradedTokenizedCard;
		[Export ("upgradedTokenizedCard")]
		BTThreeDSecureCardNonce UpgradedTokenizedCard { get; set; }

		// @property (copy, nonatomic) void (^completionBlockAfterAuthenticating)(BTThreeDSecureCardNonce *, NSError *);
		[Export ("completionBlockAfterAuthenticating")]
		Action<BTThreeDSecureCardNonce, NSError> CompletionBlockAfterAuthenticating { get; set; }
	}
	...
	interface BTUICardType
	{
		...
		string[] ValidNumberPrefixes { get; }
	...
	interface BTUITextFieldEditDelegate
	{
		...
		void TextFieldWillInsertText (BTUITextField textField, string text);

		// @optional -(void)textField:(BTUITextField *)textField didInsertText:(NSString *)text;
		[Export ("textField:didInsertText:")]
		void TextFieldDidInsertText (BTUITextField textField, string text);
	}
