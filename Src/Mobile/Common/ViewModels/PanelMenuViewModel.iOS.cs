	...
	public partial class PanelMenuViewModel
	{
		...
		private void InitDefaultIOSMenuList()
		{
			try
			{
				...
				ItemMenuList.Add(new ItemMenuModel { ItemMenuId = 10, Text = this.Services().Localize["PanelMenuViewAboutUsText"], NavigationCommand = NavigateToAboutUs });
				ItemMenuList.Add(new ItemMenuModel { ItemMenuId = 12, Text = this.Services().Localize["PanelMenuViewSignOutText"], NavigationCommand = SignOut });
			}
			catch (Exception ex)
			{
		...
		partial void InitIOSMenuList()
		{
			try
			{
				...
				{
					ItemMenuList.Add(new ItemMenuModel { ItemMenuId = 8, Text = this.Services().Localize["PanelMenuViewCallTaxiDispatchText"], NavigationCommand = CallTaxi });
				}
				if (!Settings.HideCallDispatchButton)
				{
					ItemMenuList.Add(new ItemMenuModel { ItemMenuId = 9, Text = this.Services().Localize["PanelMenuViewCallLuxuryDispatchText"], NavigationCommand = CallLuxury });
				}
				if (DisplayReportProblem)
				{
					ItemMenuList.Add(new ItemMenuModel { ItemMenuId = 11, Text = this.Services().Localize["PanelMenuViewReportProblemText"], NavigationCommand = NavigateToReportProblem });
				}
