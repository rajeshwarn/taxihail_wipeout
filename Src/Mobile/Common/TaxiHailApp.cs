    ...
    public class TaxiHailApp  : MvxApplication
    {
        ...
        private void InitalizeServices()
        {
			...
			_container.Register<IPaymentService>((c, p) =>
			{
                ...
                return new PaymentService(baseUrl, sessionId, c.Resolve<ConfigurationClientService>(), c.Resolve<IAccountService>(), c.Resolve<ICacheService>(), c.Resolve<IPackageInfo>(), c.Resolve<ILogger>());
			});
