...
namespace TaxiMobileApp
{
			...
			if (setUpComplete == false)
			{
				...
				case CAlertViewType.ActivityIndicator:
					...
					if (this.ActivityIndicator == null)
					{
						...
						if (_offset.HasValue)
						{
							this.ActivityIndicator.Frame = new RectangleF (139.0f - 18.0f, rect.Height - 63f - _offset.Value, 37.0f, 37.0f);
						}
						else
						{
	...
	public class LoadingOverlay : UIView
	{
		...
		public static void StartAnimatingLoading (UIView owner, LoadingOverlayPosition position, string text, int? width, int? height)
		{
			StartAnimatingLoading (owner, position, text, width, height, null);
		}

		public static void StartAnimatingLoading (UIView owner, LoadingOverlayPosition position, string text, int? width, int? height, Action canceled)
		{
			if (_loading == null)
			{
		...
		public static void StopAnimatingLoading (UIView owner)
		{
			if (_loading != null)
			{
//				owner.InvokeOnMainThread ( (  ) => _loading.StopAnimating (  ) );			
//				_loading.RemoveFromSuperview();
				
				owner.InvokeOnMainThread (() => _loading.Hide (false));
				owner.InvokeOnMainThread (() => _loading.RemoveFromSuperview ());
				_loading = null;
			}
		...
		public void StartAnimating (LoadingOverlayPosition position, string text)
		{
			...
			if (position == LoadingOverlayPosition.BottomRight)
			{
				x = 320 - Width - 5;
				y = 420 - 25;
			}
